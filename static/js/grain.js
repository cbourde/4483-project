...
function healFull(){
	if (checkMoney(FULL_HEAL_PRICE)){
		...
		let money = parseInt(window.localStorage.getItem("money"));
		window.localStorage.setItem("money", money - FULL_HEAL_PRICE);
...
function healHalf(){
	if (checkMoney(HALF_HEAL_PRICE)){
		...
		if (health >= maxHealth / 2){
			alert("You're already over half health!");
			return;
		}
		let money = parseInt(window.localStorage.getItem("money"));
		window.localStorage.setItem("money", money - HALF_HEAL_PRICE);
		window.localStorage.setItem("currentHealth", maxHealth / 2);
	}
}
